...
public class WeatherServiceImpl implements WeatherService {
    ...
    private static final String API_KEY = "c0a316de9155bfcbeadf592c794cdf9a";
    ...
    public AirPollutionDto getCoordsByCityName(String city, String apiKey) {
        ...
        if (response != null && response.length > 0 && response[0] != null) {
            ...
            AirPollutionDto airPollutionDto = getAirPollution(lat, lon);
    ...
    public AirPollutionDto getAirPollution(double lat, double lon) {
        return this.restTemplate.getForObject(airPollutionUrl(lat, lon), AirPollutionDto.class);
    }
    ...
    private String weatherUrl(String city, String apiKey) {
        if (apiKey != null) {
            ...
            return String.format(WEATHER_API_URL.concat("?q=%s").concat("&limit=1").concat("&appid=%s").concat("&units=metric"), city, API_KEY);
        }
    }
    ...
    private String forecastUrl(String city, String apiKey) {
        if (apiKey != null) {
            ...
            return String.format(FORECAST_API_URL.concat("?q=%s").concat("&appid=%s").concat("&units=metric"), city, API_KEY);
        }
    }

    private String airPollutionUrl(double lat, double lon) {
        return String.format(AIR_POLLUTION_API_URL.concat("?lat=%s").concat("&lon=%s").concat("&appid=%s"), lat, lon, API_KEY);
    }
    ...
    private String geocodeUrl(String city, String apiKey) {
        if (apiKey != null) {
            ...
            return String.format(GEOCODING_API_URL.concat("?q=%s").concat("&appid=%s"), city, API_KEY);
        }
